...
class Choice {
  ...
  /// agent's `❯` or `›` points at. Enter picks whatever that is.
  final bool selected;

  const Choice({required this.label, this.detail = '', this.selected = false});
...
for line in wanted:
  ...
  Future<void> _bindSelectedPane({bool silent = false, int attempt = 0}) async {
    ...
    // An empty file is indistinguishable from a slow one once we are only
    // watching the stream, so settle it here instead of guessing later.
    if (await _isEmptyFile(ssh, path)) {
      transcriptDiagnostic = 'Session file is empty:\n$path';
      _finishLoading(epoch);
      return;
    }
    if (epoch != _bindEpoch) return;
...
if found:
  ...
  Future<bool> _isEmptyFile(SSHClient ssh, String path) async {
    try {
      final out = await HerdrClient.run(ssh, 'wc -c < ${_shellQuote(path)} 2>/dev/null');
      final size = int.tryParse(utf8.decode(out).trim()) ?? -1;
      return size == 0;
    } catch (_) {
      return false;
    }
  ...
  Future<void> _startFollow(
    ...
    final command = 'tail -c +$from -f ${_shellQuote(path)}';
  ...
  static ({String question, List<Choice> choices}) parsePrompt(String raw) {
    ...
    // right, on the same lines: "2. Breakdown object   │   subtotal: number,".
    // Everything from the box edge on belongs to the preview, not the option.
    final lines = [
      for (final line in raw.split('\n'))
        line.split(RegExp(r'[│┌└┐┘]')).first.trimRight(),
    ];
    ...
    if (choices.isEmpty) {
      return (question: _liveOutputOnly(raw), choices: const <Choice>[]);
    }
  ...
  static List<String> menuKeys(
    ...
    final to = choice - 1;
    return [
      for (var i = from; i < to; i++) 'down',
      for (var i = from; i > to; i--) 'up',
      'enter',
    ];
  }
  ...
  Future<void> answerPrompt(String paneId, int choice) async {
    touchActivity();
    try {
      await _rpc?.sendKeys(paneId, menuKeys(_blockedPrompts[paneId], choice));
    } catch (_) {}
  ...
  Future<void> _refreshBlockedPrompts() async {
    ...
    for (final pane in blocked) {
      try {
        ...
        final text =
            await rpc.readPane(pane.paneId, source: 'visible', lines: 30);
